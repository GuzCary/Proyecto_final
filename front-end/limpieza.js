...
function cargarProductos() {
    fetch("../backend/usuarios/limpieza/listar_productos.php")
        .then(res => res.json())
        .then(resultado => {
            if (resultado.status === "success") mostrarProductos(resultado.productos);
        });
}
function mostrarProductos(productos) {
    ...
    cuerpo.innerHTML = productos.map(p => `
        <tr>
            ...
            <td>${p.stock}</td>
            <td>
                <button onclick="modificarStock('${p.id}', 1)">+1</button>
                <button onclick="modificarStock('${p.id}', 5)">+5</button>
                <button onclick="modificarStock('${p.id}', -1)">-1</button>
                <button onclick="modificarStock('${p.id}', -5)">-5</button>
            </td>
        </tr>
...
document.getElementById("formNuevoProducto").addEventListener("submit", function(e) {
    ...
    fetch("../backend/usuarios/limpieza/productos.php", { method: "POST", body: datos })
        .then(res => res.json())
        .then(resultado => {
...
function modificarStock(idEncriptado, cantidad) {
    ...
    fetch("../backend/usuarios/limpieza/productos.php", { method: "POST", body: datos })
        .then(res => res.json())
        .then(resultado => {
            if (resultado.status === "success") cargarProductos();
